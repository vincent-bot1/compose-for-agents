...
// ListToolsHandler is a function that handles the list tools request.
type ListToolsHandler func(ctx context.Context, request mcp.ListToolsRequest) (*mcp.ListToolsResult, error)

// ToolCallHandler is a function that handles tool calls.
type ToolCallHandler func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error)
...
// WithListToolsHandler allows bypassing the listing of tool.
func WithListToolsHandler(
	listToolsHandler ListToolsHandler,
) ServerOption {
	return func(s *MCPServer) {
		s.middlewareMu.Lock()
		s.listToolsHandler = listToolsHandler
		s.middlewareMu.Unlock()
	}
}

// WithToolCallHandler allows bypassing the tool call.
func WithToolCallHandler(
	toolCallHandler ToolCallHandler,
) ServerOption {
	return func(s *MCPServer) {
		s.middlewareMu.Lock()
		s.toolCallHandler = toolCallHandler
		s.middlewareMu.Unlock()
	}
}
